...
- `pi_ppo_lstm` depends on `components/pi_*.py`, `components/path_integration.py`, `rl-baselines3-zoo/conf/maze_pi.yml`, and zoo algorithm registration. `achieved_goal` is the PI target and should be dropped from policy features, not from rollout observations. Keep Phase 1 dataset generation and PI RL runs aligned on `sensor_aware=True`.
