...
- Keep Phase 1 fresh-model settings in `rl-baselines3-zoo/conf/maze_pi.yml`; standalone offline PI scripts should load from that config instead of duplicating model kwargs. `scripts/offline_pi_rehearsal.py --config-path` may point fresh model creation at another zoo-style config, but it does not affect checkpoint loading.
