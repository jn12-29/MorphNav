...
_XML_PATH = os.path.join(os.path.dirname(__file__), "asset/dmc_rat.xml")
...
class Rat(legacy_base.Walker):
    ...
    def _build(self, params=None, name="walker", torque_actuators=False, foot_mods=False, initializer=None):
        ...
        if foot_mods:
            self._mjcf_root.find("default", "ankle").joint.range = [-0.1, 2.0]  # default is "-0.1396263401595464 1.396263401595464"
            self._mjcf_root.find("default", "toe").joint.range = [-0.7, 0.87]   # default is "0 0.8726646259971648"
    ...
    def primary_joints(self):
        ...
        return tuple(jnt for jnt in self._mjcf_root.find_all("joint") if "vertebra" not in jnt.name)

    @composer.cached_property
    def vertebra_joints(self):
        """Return vertebra joints."""
        return tuple(jnt for jnt in self._mjcf_root.find_all("joint") if "vertebra" in jnt.name)
    ...
    def pose_to_actuation(self, pose):
        ...
        return np.hstack(
            [
                tendon_actions,
                (2 * pose[self.joint_actuator_order] - (r_lower + r_upper)) / (r_upper - r_lower),
            ]
        )
...
class RodentObservables(legacy_base.WalkerObservables):
    ...
    def egocentric_camera(self):
        ...
        return observable.MJCFCamera(
            self._entity.egocentric_camera, width=imgsize, height=imgsize, scene_option=self._scene_options
        )
