...
from utils import placement as plu
...
class Objects:
    ...
    def add_goal(self, env):
        ...
        self.goal_pos = self.place_object(
            env, [gs, gs, gh], cfg.OBJECT.GOAL_POS, center=center
        )
        self.markers.append(
            {
                "label": "",
    ...
    def add_obstacles(self, env):
        ...
        for idx in range(cfg.OBJECT.NUM_OBSTACLES):
            bl = su.sample_from_range(cfg.OBJECT.OBSTACLE_LEN_RANGE, self.np_random)
            bw = su.sample_from_range(cfg.OBJECT.OBSTACLE_WIDTH_RANGE, self.np_random)
            obstacle_dims = [bl, bw, 2]
            pos = self.place_object(env, obstacle_dims, None, modify_hfield=True)
            if not pos:
                continue
    ...
    def manipulation_obs_step(self, env, sim):
        ...
        obj_state = np.vstack((obj_rel_pos, obj_rel_vel, obj_rot_vel, goal_rel_pos))
