...
class Terrain:
    ...
    def _create_rugged_square(self, nrow, ncol):
        ...
        idx = 0
        while True:
            width = su.sample_from_list(range(10, 20, 1), rng_state=self.np_random)
    ...
    def _create_steps(self, nrow, ncol):
        ...
        hfield_data = np.zeros((nrow, ncol))
        heights = list(range(0, num_steps, 1)) + list(range(num_steps - 2, -1, -1))
    ...
    def create_boundary_walls(self):
        ...
        boundary.append(xu.floor_segm("boundary/front", pos, size, "box", "boundary"))
        # Add back wall
        pos = [-(cfg.TERRAIN.START_FLAT + WALL_LENGTH), 0, wall_height - h]
        boundary.append(xu.floor_segm("boundary/back", pos, size, "box", "boundary"))
        # Add right wall
        pos = [center, w + WALL_LENGTH, wall_height - h]
        size = [l, WALL_LENGTH, wall_height]
        boundary.append(xu.floor_segm("boundary/right", pos, size, "box", "boundary"))

        # Add left wall
        pos = [center, -(w + WALL_LENGTH), wall_height - h]
        boundary.append(xu.floor_segm("boundary/left", pos, size, "box", "boundary"))
        # Modify hfield
    ...
    def create_arena(self):
        ...
        # Each iteration of the loop will add one flat segm, and one randomly
        # sampled obstacle from the list cfg.TERRAIN.TYPES
        self.max_lim = self.hfield.shape[1] / 2 - cfg.TERRAIN.CENTER_FLAT * self.divs
        while self.segms[-1]["end_x"] < cfg.TERRAIN.SIZE[0] * 2:
            trr_type = su.sample_from_list(self.terrain_types, self.np_random)
            obstacle = self._create_terrain_segm(trr_type)
            flat = self._create_terrain_segm("flat")
            ...
            if self.segms[-1]["end_x"] == cfg.TERRAIN.SIZE[0] - cfg.TERRAIN.CENTER_FLAT:
                self.max_lim = self.hfield.shape[1]
                xml_elems.append(
                    self._create_terrain_segm("flat", size=cfg.TERRAIN.CENTER_FLAT)[0]
                )
