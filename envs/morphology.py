...
class SymmetricUnimal:
    ...
    def _construct_head(self):
        ...
        head.append(xu.site_elem("torso/touch/0", None, "touch_site", str(r + 0.01)))
    ...
    def _construct_limb(self, idx, site, site_type, limb_params, orient=None):
        ...
        limb.append(
            xu.site_elem("limb/mid/{}".format(idx), [x_mid, y_mid, z_mid], site_type)
        )

        x_end, y_end, z_end = gu.sph2cart(r + h, theta, phi)
        limb.append(
            xu.site_elem("limb/btm/{}".format(idx), [x_end, y_end, z_end], site_type)
        )
    ...
    def _attach(self, site, body_part):
        parent = self.unimal.find('.//site[@name="{}"]...'.format(site.get("name")))
    ...
    def mutate_density(self):
        self.body_params["limb_density"] = su.sample_from_range(cfg.LIMB.DENSITY_RANGE)
            ...
            for insert_pos, axis_ in enumerate(joint_axis):
                body.insert(
                    ...
                    xu.joint_elem(
                        ...
                        range_=xu.arr2str(joint_params["joint_range"][insert_pos]),
                        pos=joint_pos,
                    ),
                )
                self.limb_metadata[limb_idx]["gear"][axis_] = joint_params["gear"][
                    insert_pos
                ]
    ...
    def _get_new_limbs(self, site, limbs2add, limb_params):
        ...
        if site_type == "growth_site":
            ...
            mirror_site = xu.find_elem(self.unimal, "site", "name", mirror_site_name)[0]
    ...
    def _choose_site(self):
        ...
        torso = xu.find_elem(self.root, "body", "name", "torso/{}".format(torso_idx))[0]
        free_sites = xu.find_elem(torso, "site", "class", "growth_site")
        free_sites.extend(xu.find_elem(torso, "site", "class", "mirror_growth_site"))
        ...
        if site.get("class") == "growth_site":
            ...
            # prevents duplicates.
            mirror_site_name = self.mirror_sites[site.get("name")]
            if mirror_site_name < site.get("name") and "torso" not in mirror_site_name:
                site = xu.find_elem(self.unimal, "site", "name", mirror_site_name)[0]
            limbs2add = 2
    ...
    def _add_sensors(self):
        ...
        sensor.append(
            etree.Element("subtreeangmom", {"name": "unimal_am", "body": "torso/0"})
        )
        # Add touch sensors
    ...
    def save(self):
        ...
        save_path = os.path.join(cfg.OUT_DIR, "unimal_init", "{}.pkl".format(self.id))
