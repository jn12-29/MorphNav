...
import utils.mjpy as mu
from config import cfg
from envs.modules.agent import Agent
from envs.modules.objects import Objects
...
from envs.wrappers.metrics import ManipulationMetric
...
class ManipulationTask(UnimalEnv, utils.EzPickle):
    ...
    def _cal_agent_obj_dist(self):
        ...
        if self.obj_type == "box":
            ...
            obj_pos = [self.sim.data.get_body_xpos(self.obj_name).copy()]
    ...
    def step(self, action):
        ...
        if agent_obj_d_after <= cfg.OBJECT.SUCCESS_MARGIN and not self.reached_obj:
            reach_reward += 10.0
