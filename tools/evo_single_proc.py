...
from envs.morphology import SymmetricUnimal
from utils import evo as eu
from utils import exception as exu
...
def ppo_train(xml_file, id_, parent_metadata=None):
    ...
    if (
        exit_cond == "population_init"
        and eu.get_population_size() >= cfg.EVO.INIT_POPULATION_SIZE
    ):
        return

    if (
        exit_cond == "search_space"
        and eu.get_searched_space_size() >= cfg.EVO.SEARCH_SPACE_SIZE
    ):
        return
...
def init_done(unimal_id):
    ...
    done_idx = [
        int(path.split("/")[-1].split("-")[2].split(".")[0]) for path in done_metadata
    ]
...
def init_population(proc_id):
    ...
    for xml_path in xml_paths:
        ...
        print(f"Now process {unimal_id}, proc_id: {proc_id}, xml_path: {xml_path}")
...
def tournament_evolution(idx):
    ...
    while eu.get_searched_space_size() < cfg.EVO.SEARCH_SPACE_SIZE:
        ...
        unimal = SymmetricUnimal(
            child_id,
            init_path=fu.id2path(parent_metadata["id"], "unimal_init"),
        )
        unimal.mutate()
        unimal.save()
    ...
    video_meta_files = fu.get_files(video_dir, "{}-{}-.*json".format(cfg.NODE_ID, idx))
