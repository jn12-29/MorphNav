...
from utils import similarity as simu
    ...
    while eu.get_searched_space_size() < cfg.EVO.SEARCH_SPACE_SIZE:
        ...
        for idx in range(len(subprocs)):
            ...
            if os.path.exists(is_error_path) or poll:
                ...
                fu.remove_file_and_backup_file(is_error_path)
                info_str = f"Node ID: {cfg.NODE_ID}, proc-id: {proc_id}"
                info_str += (
                    f", error file path : {is_error_path }"
                    if os.path.exists(is_error_path)
                    else ", finished"
                )
                print(info_str)
